...
void executer(const std_msgs::Char::ConstPtr& key){
  ...
  if (key->data == 'a' || key->data == 'A') {
	...
	std::system("/home/dud/ros_ws/src/INEX/src/transmit.sh");
	ROS_INFO("[Transmitting data]");
  };
}
...
int main(int argc, char *argv[]) {
    ...
    ros::subscriber power = nh.subscribe("power", 5, low_power); //must find batery topic
    ros::spin();
}
